        ...
        float Kp, float Ki, float Kd, int POn, int ControllerDirection)
{
    ...
    mySetpoint = Setpoint;
    inAuto = false;
    PID::SetIntegralLimits(-3.4028235e38, 3.4028234e38);
...
bool PID::Compute()
{
   ...
   if(timeChange>=SampleTime)
   {
      ...
      if(outputSum > min(outMax, integralMax)) outputSum= min(outMax, integralMax);
      else if(outputSum < max(outMin, integralMin)) outputSum= max(outMin, integralMin);
